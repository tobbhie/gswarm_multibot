...
GSWARM_API_BASE = "https://gswarm.dev/api"
GSWARM_API_SECRET = os.getenv("GSWARM_API_SECRET")
    ...
    try:
        while True:
            ...
            if verify_re.search(line):
                ...
                # Auto-call API with code only; backend infers Discord ID
                try:
                    if not GSWARM_API_SECRET:
                        await send_safe(chat_id, f"🔐 Detected code `{code}`. Server missing GSWARM_API_SECRET; cannot link.", parse_mode="Markdown")
                    else:
                        async with httpx.AsyncClient(timeout=15) as client:
                            resp = await client.post(
                                f"{GSWARM_API_BASE}/discord/link",
                                json={"code": code},
                                headers={"X-API-SECRET": GSWARM_API_SECRET},
                            )
                        if resp.status_code == 200:
                            ...
                            await send_safe(
                                ...
                                (
                                    ...
                                    + (f"Role granted: `{role_id}`" if role_id else "Role granted.")
                                ),
                                parse_mode="Markdown",
                            )
                            await stop_active_session("✅ Account linked successfully.")
                        elif resp.status_code == 400:
                            await send_safe(chat_id, "❌ Invalid or expired code. Please request a new one.")
                        elif resp.status_code == 401:
                            await send_safe(chat_id, "❌ Authorization failed. Please contact the admin.")
                        else:
                            await send_safe(chat_id, f"⚠️ API error ({resp.status_code}). Please try again later.")
                except httpx.RequestError as e:
                    await send_safe(chat_id, f"⚠️ Network error contacting API: {e}")
                except Exception as e:
                    await send_safe(chat_id, f"⚠️ Unexpected error: {e}")
...
async def handle_message(message: types.Message):
    ...
    if text.lower().startswith("/verify"):
        ...
        _, code = parts
        # Basic validation
        if not (len(code) == 8 and code.isalnum() and code.upper() == code):
            await message.answer("⚠️ Code must be 8 characters (A-Z, 0-9).")
            return
        if not GSWARM_API_SECRET:
            await message.answer("⚠️ Server misconfigured: GSWARM_API_SECRET is not set.")
            return

        # Call GSwarm API to link with code only
        try:
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
                    f"{GSWARM_API_BASE}/discord/link",
            ...
            if resp.status_code == 200:
                ...
                # End session if this user owns it
                if active_session.get("chat_id") == chat_id:
                    await stop_active_session("✅ Account linked successfully.")
            elif resp.status_code == 400:
                await message.answer("❌ Invalid or expired code. Please request a new one.")
            elif resp.status_code == 401:
                await message.answer("❌ Authorization failed. Please contact the admin.")
            else:
                await message.answer(f"⚠️ API error ({resp.status_code}). Please try again later.")
        except httpx.RequestError as e:
            await message.answer(f"⚠️ Network error contacting API: {e}")
        except Exception as e:
            await message.answer(f"⚠️ Unexpected error: {e}")
        return
